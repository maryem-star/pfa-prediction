...
const AnalysisPage = () => {
  ...
  useEffect(() => {
    api.get("/dashboard/stats")
      ...
      .catch(() => setStats({ total_etudiants: 0, taux_reussite: 0, taux_moyen: 0, taux_risque: 0 }));
  ...
  const filiereStats = analysis?.filiere_stats || [];
    ...
    <Layout>
      <div className="min-h-full p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          <StatCard delay={0.05} color="#1e56a0" label="Total Etudiants" value={stats?.total_etudiants || 0} sub="Toutes filieres"
            icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>} />
          <StatCard delay={0.1} color="#16a34a" label="Taux Reussite" value={`${stats?.taux_reussite || 0}%`} sub="Predictions ML"
          ...
          <StatCard delay={0.15} color="#d97706" label="Taux Moyen" value={`${stats?.taux_moyen || 0}%`} sub="Suivi recommande"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={filiereStats.length > 0 ? filiereStats : []} barSize={20}>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={moduleAvg.length > 0 ? moduleAvg : []} barSize={14} layout="vertical">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={absencesData.length > 0 ? absencesData : []} barSize={40}>
