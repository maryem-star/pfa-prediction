...
const ROLE_LABELS = {
  super_admin: { label: "Super Administrateur", color: "#1e56a0", bg: "#eff6ff", border: "#bfdbfe" },
  directeur_adjoint: { label: "Directeur Adjoint", color: "#0f766e", bg: "#f0fdfa", border: "#99f6e4" },
  chef_departement: { label: "Chef de Département", color: "#7c3aed", bg: "#f5f3ff", border: "#ddd6fe" },
  chef_filiere: { label: "Chef de Filière", color: "#b45309", bg: "#fffbeb", border: "#fde68a" },
};
...
const LoginPage = () => {
  ...
  // Rôle choisi depuis la HomePage
  const roleHint = location.state?.roleHint;
  const roleInfo = ROLE_LABELS[roleHint] || null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Redirection dynamique selon le vrai rôle retourné par le backend
    ...
    <div className="relative min-h-screen overflow-hidden flex items-center justify-center"
      ...
      >
        <div className="relative bg-white/85 backdrop-blur-xl border border-white/70 rounded-2xl p-8 shadow-[0_20px_60px_rgba(0,0,0,0.1)]">
          ...
          {/* Top gradient bar — couleur selon le rôle */}
          <div className="absolute top-0 left-0 right-0 h-1 rounded-t-2xl"
            style={{ background: roleInfo
              ? `linear-gradient(90deg, ${roleInfo.color}, #e87722)`
              : "linear-gradient(90deg, #1e56a0 0%, #e87722 100%)"
            }} />
          ...
          {/* Badge rôle (si vient de la HomePage) */}
          {roleInfo && (
            <motion.div
          ...
          <motion.form onSubmit={handleSubmit} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <motion.button type="submit" disabled={loading} whileTap={{ scale: 0.98 }}
              ...
              style={{ background: roleInfo
                ? `linear-gradient(135deg, ${roleInfo.color}, #2568b5)`
                : "linear-gradient(135deg, #1e56a0, #2568b5)"
              }}>
              {loading ? (
          ...
          <p className="text-center text-xs text-gray-300 mt-5">
            {roleInfo ? `Connexion — ${roleInfo.label}` : "Accès réservé au staff"}
          </p>
        </div>
